...
public class #类名# : MonoBehaviour
{
    ...
    //初始化参数
    public overrid void IniParameter(){
        if(objs.Length>0){
...
";
    ...
    public static void BuildUIScript()
    {
        ...
        if(selectobj.name.StartsWith("Scene_")){
            ...
            fileW.Write(sceneScriptStr);
            fileW.Flush();
            fileW.Close();
            file.Close();

            //创建Prefab
            string prefabPath = Application.dataPath + "/TResources/Prefabs/UIMainPrefabs/Scenes/";
            ...
            if(createPrefabSucceed) 
                TLogger.LogInfo("创建scene预制体成功!"+prefabPath+selectobj.name);
