...
public class ZZTipManager : UnitySingleton<ZZTipManager>
{
    ...
    private GameObject createTipInTo(string tipName,Transform parent){
        GameObject go = TResources.Load("Prefabs/UIMainPrefabs/Tips/"+tipName+".prefab",parent);
        go.rectTransform().localPosition = Vector3.zero;
        return go;
    }

    private void ShowAniAndClose(GameObject tip,TipShowType showType,Action<bool> isClose = null){
        RectTransform rt = tip.rectTransform();
